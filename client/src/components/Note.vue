    ...
    <div class="flex items-center justify-between">
      ...
      <div v-if="!isMobile" class="group relative text-left">
        ...
        <div class="invisible absolute -right-1 z-10 w-[148px] origin-top-right group-hover:visible" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
          <a href="#" @click="exportNote" class="flex items-center rounded-t-md border-2 border-b-0 border-neutral-700 bg-white p-2 pb-[5px] pt-[5px] text-base text-neutral-700 hover:bg-neutral-700 hover:text-slate-50 dark:border-neutral-700 dark:bg-neutral-800 dark:text-stone-200 hover:dark:bg-white hover:dark:text-neutral-800" role="menuitem" tabindex="-1" id="menu-item-0">
          ...
          <a href="#" @click="excludeClick = true" class="flex items-center rounded-b-md border-2 border-neutral-700 bg-white p-2 pb-[5px] pt-[5px] text-base text-neutral-700 hover:bg-neutral-700 hover:text-slate-50 dark:border-neutral-700 dark:bg-neutral-800 dark:text-stone-200 hover:dark:bg-white hover:dark:text-neutral-800" role="menuitem" tabindex="-1" id="menu-item-0">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="mr-[5px] size-[22px]">
...
export default {
  ...
  setup(props) {
    ...
    const tagColor = props.tag.length === 0 ? "" : store.user.tags.filter((tag) => tag.name === props.tag).map((tag) => colorsMap[tag.color])
    ...
    return { deleteButton, editButton, isMobile, tagColor, store, showButtons, position, selected, mirrorButtons, pressStartTime, longPress, excludeClick }
  },
  methods: {
    startTouch(event) {
    ...
    handleTouch(event) {
      ...
      if (newPosX > editButtonPosX && newPosX < editButtonPosX + 48 && newPosY > editButtonPosY && newPosY < editButtonPosY + 48) this.selected = "export"
      else if (newPosX > deleteButtonPosX && newPosX < deleteButtonPosX + 48 && newPosY > deleteButtonPosY && newPosY < deleteButtonPosY + 48) this.selected = "delete"
      else this.selected = null
    },
    endTouch() {
      if (this.selected === "export") this.exportNote()
      else if (this.selected === "delete") alert("Elimina nota") // Aggiungere funzione elimina
    ...
    exportNote() {
      ...
      a.href = url
      a.download = this.title.toLowerCase() + ".md"
      a.click().remove()
