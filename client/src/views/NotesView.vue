<template>
  <div class="flex min-h-screen flex-col">
    ...
    <div class="ml-5 mr-5 grid h-full grid-cols-1 gap-y-[10px] overflow-auto">
      <div class="tracking-tigh w-full rounded-2xl bg-neutral-800 p-4 pb-3 pt-3" @touchstart="startTouch" @touchmove="moveTouch" @touchend="endTouch" style="user-select: none">
        ...
        <div v-if="showButtons" class="absolute flex flex-col items-center gap-3 rounded-lg transition-transform" :style="{ top: `${position.y}px`, left: `${position.x}px` }">
          <button class="rounded-full bg-neutral-900 text-slate-50 p-3" :class="{ 'scale-125': selected === 'edit' }">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
              <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
            </svg>
          </button>
          <button class="rounded-full bg-neutral-900 text-slate-50 p-3 " :class="{ 'scale-125': selected === 'delete' }" :style="{ transform: 'translateX(50px)' }">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
...
export default {
  ...
  methods: {
    startTouch(event) {
      ...
      this.position = { x: touch.clientX + 5, y: touch.clientY - 100 }
      this.startY = touch.clientY
    },
    moveTouch(event) {
      const touch = event.touches[0]
      const deltaY = touch.clientY - this.startY
      this.selected = deltaY < -20 ? "edit" : deltaY > 20 ? "delete" : null
    },
    endTouch() {
...
/*
Note per l'animazione:
Bisogna blurrare lo schermo tranne la nota selezionata quando si tiene premuto sopra la nota
Bisogna che l'animazione si attivi solo se si tratta di long press
Il rilascio del touch deve attivare l'azione solo se si è esattamente sopra uno dei due bottoni
*/
